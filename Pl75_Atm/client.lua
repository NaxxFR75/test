	...
	options = {
        {
            action = function()
                if crowbar() then
                    TriggerEvent('ledjo-atmheist:StartDestroy')
                end
            end,
			icon = "fas fa-hammer",
			label = "Braquer L\'Atm",
...
RNE('ledjo-atmheist:StartHack')
AEH('ledjo-atmheist:StartHack', function()
    ESX.TriggerServerCallback('ledjo-atmheist:PoliceCount', function(status)
        if status then
            ESX.TriggerServerCallback('ledjo-atmheist:CheckTime', function(time)
                if time then
                    ...
                    exports['ledjo_lib']:Thermite(function(success)
                        if success then
                            if lib.progressBar({
                                ...
                                TSE('ledjo-atmheist:Reward', 'full')
                            else
                                TSE('ledjo-atmheist:Reward', 'half')
...
RNE('ledjo-atmheist:StartDestroy')
AEH('ledjo-atmheist:StartDestroy', function()
    ESX.TriggerServerCallback('ledjo-atmheist:PoliceCount', function(status)
        if status then
            ESX.TriggerServerCallback('ledjo-atmheist:CheckTime', function(time)
                if time then
