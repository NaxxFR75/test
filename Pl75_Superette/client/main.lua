...
function checkConditions()
    ...
    if hasItem then
        if Config.RequirePolice then
            local policeCount = lib.callback.await('ledjo_superette:getPoliceCount', false)
        ...
        if cooldown == false then
            ...
            if success then
                ...
                if lib.progressCircle({
                    ...
                    if codeChance <= Config.CodeChance then
                        ...
                        if note == 'confirm' then
                            ...
                            activeSafe = true
                            verifyReward = true
                            local reward = lib.callback.await('ledjo_superette:registerSuccessful', false, verifyReward)
                            if reward then
                                verifyReward = false
                            else
                ...
                if lockpickBreakChance <= Config.LockpickBreakChance then
                    TriggerServerEvent('ledjo_superette:removeItem', cache.serverId, Config.RegisterRobberyItem, 1)
    ...
    if wrongPIN < Config.MaxCodeAttempts then -- Checks PIN attempts
        ...
        elseif convertedCode == safePin then -- Correct PIN
            ...
            if lib.progressCircle({
                ...
                activeRegister = false
                verifyReward = true
                local reward = lib.callback.await('ledjo_superette:safeSuccessful', false, verifyReward)
                if reward then
                    verifyReward = false
                else
                    -- Kick/drop player? Potential cheating?
