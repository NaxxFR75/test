...
RSC('treespace-atmheist:PoliceCount', function(source, cb)
    local src = source
...
RSC('treespace-atmheist:CheckTime', function(source, cb)
    local src = source
    ...
    if (os.time() - lastrob) < Config.Cooldown and lastrob ~= 0 then
        ...
        lastrob = os.time()
      --  exports['treespace-logs']:SendLog(source, 'Il a commencé à voler un guichet automatique!', 'crime')
        cb(true)
    end
end)

RSE('treespace-atmheist:Reward')
AEH('treespace-atmheist:Reward', function(type)
