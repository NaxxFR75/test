...
function showMenu(show)
    ESX.TriggerServerCallback("blackMarketgn:getBlackMoney", function(black_money)
...
RegisterNUICallback("payWeapon", function(data, cb)
    local weapon = data
    TriggerServerEvent("blackMarketgn:giveTools", "weapons", weapon.name, weapon.price)
...
RegisterNUICallback("payItem", function(data, cb)
    ...
    TriggerServerEvent("blackMarketgn:giveTools", "items", item.name, item.price)
    ...
    while true do
        ...
        for k, position in ipairs(Config.Positions) do
            ...
            if dist <= 10.0 then
                ...
                if dist <= 2.0 then
                    wait = 0
                    ESX.ShowHelpNotification("Appuyer sur ~b~[E]~s~ pour interagir")
