...
AddEventHandler("Tikoz:BlanchimentJob", function(amount)
    ...
    if amount > 0 and cbsale >= amount then
        ...
        TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, 'Blanchiment', "~b~Argent sale", "Vous blanchissez : ~r~"..amount.."€", 'CHAR_BLANK_ENTRY', 9)

        Citizen.Wait(15000)

        xPlayer.addMoney(propre)
        TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, 'Blanchiment', "~b~Argent sale", "Vous avez récupéré : ~g~"..propre.."€", 'CHAR_BLANK_ENTRY', 9)
    else
        TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, 'Blanchiment', "~b~Argent sale", "Vous avez pas assez ~r~d'argent sale", 'CHAR_BLANK_ENTRY', 9)
    end
end)
