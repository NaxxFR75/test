...
local open = false 
local accueilgrandebornes = RageUI.CreateMenu('~b~grandeborne', Config.pastouche) 
...
function accgrandeborne() 
    if open then 
        ...
        RageUI.Button("Appellé un grandeborne", nil, {RightLabel = "→"}, not codesCooldown52 , {
            onSelected = function()
            codesCooldown52 = true 
        TriggerServerEvent('accueil:grandeborne')
        ESX.ShowNotification('~g~Votre message a bien été envoyé aux membre des grandeborne.')
    ...
    RageUI.IsVisible(sousacceil, function()
        ...
        RageUI.Separator('~b~Informations Recrutement grandeborne')
        RageUI.Separator('↓')
    ...
    while true do
      ...
      local wait = 500
        ...
        for k in pairs(posacgrandeborne) do
            ...
            if dist <= 3.0 then
               wait = 0
                Visual.Subtitle("Appuyez sur [~b~E~w~] pour parler à un ~b~grandeborne", 1) 
