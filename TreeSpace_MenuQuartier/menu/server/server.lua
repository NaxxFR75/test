...
ESX.RegisterServerCallback('quartier:getOtherPlayerData', function(source, cb, target)
...
ESX.RegisterServerCallback('quartier:getOtherPlayerData', function(source, cb, target, notify)
    local xPlayer = ESX.GetPlayerFromId(target)
    TriggerClientEvent("esx:showNotification", target, "~r~Vous etes fouillé...")
    if xPlayer then
        local data = {
